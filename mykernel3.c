	...
	int p, s;
{
	...
	semtab[s].value--;
        ...
        if (semtab[s].value < 0 && semtab[s].count != MAXPROCS) {
                semtab[s].waitlist[semtab[s].end] = p;
                semtab[s].end = (semtab[s].end + 1) % MAXPROCS;
                semtab[s].count++;
                Block(p);
        }
}
	...
	int p, s;
{
	...
	semtab[s].value++;
        ...
        if (semtab[s].count != 0) {
                Unblock(semtab[s].waitlist[semtab[s].begin]);
                semtab[s].begin = (semtab[s].begin + 1) % MAXPROCS;
                semtab[s].count--;
        }
}
